...
/*
功能：由角alpha及其导数计算卫星绕Z轴旋转的角度、角速度
输入：alpha是太阳在轨道系中的方位角度、UNW轨道角速度ω_U
     beta为太阳与轨道面的夹角
     PHI为轨控偏航角
输出：偏航角p，偏航角速度dp
*/
double p,dp,wu;
void PsipCal(double alpha,double beta,double PHI)
{
	double Sgx = -cos(beta)*cos(alpha);
	double Sgy = cos(PHI)*sin(beta) - sin(PHI)*cos(beta)*sin(alpha);
	double Sgz = sin(PHI)*sin(beta) + cos(PHI)*cos(beta)*sin(alpha);
	double delta1 = Sgx*Sgx + Sgy*Sgy;
	p = atan2(Sgy,Sgx)*DEG;
	dp = cos(beta)/delta1*(cos(beta)*sin(PHI)-sin(alpha)*sin(beta)*cos(PHI))*wu;
}
/*
功能：偏航角规划函数
输入：alpha是太阳在轨道系中的方位角(单位为弧度)、UNW轨道角速度ω_U
	beta为太阳与轨道面的夹角，单位为弧度
	PHI为轨控偏航角，单位为弧度
	规划参数dal、betalim，单位为度
输出：偏航角p，偏航角速度dp，是否处于偏航规划弧段内的标志bINPSIProgram
*/
int PsiProgram(double alpha,double beta,double PHI,double dal,double betalim)
{
	double alf = modpi(alpha);
	double falpha,betag;
	int bINPSIProgram;
	if(fabs(beta-PHI)<betalim*RAD && fabs(alf-PI/2)<dal*RAD)
	{
		falpha = cos(90.0/dal*(alpha-PI/2))*cos(90.0/dal*(alpha-PI/2));
		falpha = falpha*falpha;
		betag = beta + (PHI + sign(beta-PHI)*betalim*RAD - beta)*falpha;
		bINPSIProgram = 1;
	}
	else
	{
	...
	PsipCal(alpha,betag,PHI);
	return bINPSIProgram;
}
