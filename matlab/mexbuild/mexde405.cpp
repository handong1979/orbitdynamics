...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	try{
		if((PLANET)icent==NUTATIONS)
		{
			...
			DE405::Instance()->Nutations(epoch,*out,*(out+1));
		}
		else if((PLANET)icent==LIBRATIONS)
		{
			plhs[0] = mxCreateDoubleMatrix(1,3,mxREAL);
			double* out = mxGetPr(plhs[0]);
			DE405::Instance()->Librations(epoch,*out,*(out+1),*(out+2));
		}
		else
		{
			...
			double* out = mxGetPr(plhs[0]);
			DE405::Instance()->PlanetEphemeris(epoch,(PLANET)icent,(PLANET)itar,out);
		}
	}
